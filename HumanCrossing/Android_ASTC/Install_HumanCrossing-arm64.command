...
$ADB $DEVICE install HumanCrossing-arm64.apk
if [ $? -eq 0 ]; then
	echo
	$ADB $DEVICE shell pm list packages com.YourCompany.HumanCrossing
	...
	$ADB $DEVICE shell 'rm -r $EXTERNAL_STORAGE/Download/obb/com.YourCompany.HumanCrossing'
...
if [ 1 ]; then
	...
	$ADB $DEVICE shell pm grant com.YourCompany.HumanCrossing android.permission.READ_EXTERNAL_STORAGE >/dev/null 2>&1
	$ADB $DEVICE shell pm grant com.YourCompany.HumanCrossing android.permission.WRITE_EXTERNAL_STORAGE >/dev/null 2>&1
